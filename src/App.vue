...
<template>
    <header>
        ...
        <div class="nav-icon-left" title="Einen Moment warten ..." :class="{ 'active': isMenuOpen }"
            @animationend="toggle">
            <span class="material-symbols-outlined" v-if="isMenuOpen">
                close
            </span>
            <span v-if="isMenuOpen">MENÜ SCHLIESSEN</span>
            <span class="material-symbols-outlined" v-if="!isMenuOpen">
                menu
            </span>
            <span v-if="!isMenuOpen">MENÜ ÖFFNEN</span>
        </div>
...
.nav-icon-left {
    ...
    padding: 10px;
    border-radius: 50px;
    background: linear-gradient(to right, var(--color-background-mute) 50%, white 0);
    ...
    &:hover {
        animation: makeItfadeIn 1s 0.5s forwards;
    }
